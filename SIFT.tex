...
\begin{document} 
	...
	The main goal of the Scale Invariant Feature Transform is finding distinct points in an image which are likely to appear in a different image, depicting the same object(s). These distinct points are called keypoints and are being used to compute a feature descriptor which describe the local neighbourhood around that keypoint's location.  
	...
	Lowe proposed a way of how scale-invariant features can be used to recognize objects even under difficult conditions. A Hough transformation helps to detect objects even if they are partially occluded. Furthermore, SIFT can also be used for tracking objects in videos. 
