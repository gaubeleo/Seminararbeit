...
\begin{document} 
	...
	\begin{figure}[h]
		\begin{center} 
			...
			In (b) an "X"-marked pixel in one of these DoGs in being compared to its 26 neighbours, including those lying in adjacent DoGs. Only if that pixel holds the highest or lowest value of its surroundings it is considered to be a scale-space maximum or minimum, respectively. Images taken from \cite{SIFT}} 
		\end{center} 
	\end{figure} 
	...
	\begin{figure}[h]
		\begin{center} 
			...
			\caption{This figure shows an illustation of a 3D quadratic surface. A red "x" marks a sample point of the scale-space function $D$ and the green "x" marks the precise peak of the surface.
			Image taken from \cite{quadraticFunction}}
	...
	The gradient neighbourhood is split up into 4x4 cells, so there will be a total of 16 cells. Analogous to section 2.4 each cell accumulates its gradients in an orientation histogram, but this time with only 8 bins, which keeps the final descriptor small.
	...
	Experiments show that normalizing the vector to unit length helps to cope with illumination changes. By limiting every orientation value in the cell histogram to a maximum of 0.2, we guarantee that no single dominant gradient can suppress the other gradients. Have multiple strong orientations in our final descriptor is more desirable than having a single dominant one, because "non-linear illumination changes can also occur due to camera saturation or due to illumination changes that affect 3D surfaces with differing orientations by different amounts" \cite[p. 15]{SIFT}. 
	
	%\section{(Matching)} 
	
	\section{Problem cases} 
	[We have seen that SIFT is a sophisticated algorithm], but computing SIFT features on large images takes 
	...
	%\begin{figure}[h]
	%	\begin{center} 
	%		\includegraphics[width=6.5cm]{images/fence.png} 
	%		\label{subfig:fence} 
	...
	Repetative structures are not he 
