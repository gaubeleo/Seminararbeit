...
\begin{document} 
	...
	\begin{abstract} 
		...
		The following paper is supposed to give an overview of how the Scale Invariant Feature Transform (SIFT) works. It was introduced by David G. Lowe in 1999 and enhanced multiple times until he published his final version in 2004 \cite{SIFT}. This algorithm extracts distinctive features from images which can for example be used for matching objects across multiple images.
		 
		The first part of this paper introduces the basic concepts of SIFT and also presents a range of possible applications. Subsequently, every essential step that leads to the creation of these features is being explained in detail. In contrast to all the advantages of using SIFT, some of its disadvantages are being examined in the third section. [Finally, it concludes with a brief summary of the entire paper. ]
	...
	It is also possible to use SIFT for recognizing human gestures \cite{humanGestures}, which serves to control applications or even tell robots what to do. 
	
	In 2007 Lowe collaborated with Matthew Brown and together they published a paper on how to use correspondences of these invariant features to stitch partially overlapping images together, resulting in a single panoramic image \cite{panorama}. 
	...
	Lowe has found that using an initial scale $\sigma = 1.6$ and gradually adjusting each following scale of the Gaussian blur by a constant factor of $k = 2^{1/3}$ will achieve the best results \cite{SIFT}. 
	...
	\begin{figure}[h]
		\begin{center} 
			...
			In (b) an "X"-marked pixel in one of these DoGs in being compared to its 26 neighbours, including those lying in adjacent DoGs. Only if that pixel holds the highest or lowest value of its surroundings it is considered to be a scale-space maximum or minimum, respectively. Images were taken from \cite{SIFT}} 
		\end{center} 
	\end{figure} 
	...
	Our next step in the SIFT algorithm is to predict the precise location and scale of our previously extracted scale-space extrema. As an illustration example every image in DoG pyramid can also be represented as a 3D surface with pixel intensity values determining the expansion in the "z"-direction and interpolated values in between pixels as shown in Figure 3. We can improve our keypoints on a sub-pixel precision level by determining the exact locations of the peaks in these DoG surfaces.
	
	Let $\vec{x} = (x, y, \sigma)^T$ be the location and scale of a keypoint that we obtained in the previous section. For every such keypoint, we calculate a second-order Taylor expansion of the scale-space function, $D(x, y, \sigma)$, shifted by $\vec{x}$, so that the origin is at the keypoint's position.
	...
	By calculating the derivative of this function with respect to $\vec{x}$ and setting it to zero, we receive another vector $\vec{\hat x}$, which consist of the precise location and scale of our keypoint. %\cite{SIFT}??
	
	\begin{figure}[h]
		\begin{center} 
			\includegraphics[width=10cm]{images/keypoint-refinement.png} 
			\caption{This figure shows an illustation of a 3D quadratic surface. A red "x" marks a sample point of the scale-space function $D$ and the green "x" marks the precise peak of the surface. Image was taken from \cite{quadraticFunction}}
			\label{subfig:quadraticFunction} 
		\end{center}
	\end{figure}
	...
	A low contrast also means that we might not find corresponding features in an image that has slightly been modified from the original. 
	
	We can measure a keypoint's contrast by evaluating the previously used Taylor expansion of the scale-space function \eqref{eq:taylor} at $\vec{\hat x}$. Lowe found that if the abolute value of that function is less than 0.03, then there is not enough contrast for a stable feature to be computed at that location. %(normalized pixel values [0..1]) 
	...
	To further illustrate this problem, we can consider a abstract version of the Aperture Problem \cite{aperture} by imagining at an arbitrary thick white line on black background as shown in Figure 3(a). If we were to cut a patch from the middle section of the line (see Figure 3(b)), it would be impossible to tell at which exact location on the line the patch originated from. A patch of one of the end sections, as shown in 3(c), can easily be matched to its correct location at the corresponding end of the line. 
	
	In order to mathematically determine whether a given keypoint is corner-like and therefore stable or not, we can use a 2x2 Hessian matrix $H$, where $D_{xx}$, $D_{xy}$, $D_{yx}$ and $D_{yy}$ are second derivatives of the scale-space function $D$:
	...
	For efficiency purposes we can approximate these derivatives by simply subtracting neighboring sample points. Given the Hessian $H$, we can look at the ratio of its two eigenvalues which are proportional to the principal curvatures of $D$. We will only keep the keypoint if both eigenvalues are within the same magnitude. However, if one eigenvalue is significantly bigger than the other, we will discard this keypoint from our set, because we might not be able to localize it in a different scene due to only a single strong principle curvature at this keypoint.
	
	\begin{figure}[h]
		\begin{center} 
			\begin{subfigure}{0.31\textwidth} 
				\includegraphics[width=4.7cm]{images/straight_line.png} 
				\caption{}
				\label{subfig:straigh_line} 
			\end{subfigure} 
			~ 
			\begin{subfigure}{0.31\textwidth} 
				\includegraphics[width=4.7cm]{images/line_patch.png} 
				\caption{}
				\label{subfig:center_patch} 
			\end{subfigure} 
			~ 
			\begin{subfigure}{0.31\textwidth} 
				\includegraphics[width=4.7cm]{images/corner_patch.png} 
				\caption{}
	...
	There are many possibilities to describe the area around a keypoint. For instance we could be storing the intensity or color values inside our feature descriptor. However, the disadvantage of this approach would be that many images are taken under different lighting conditions. Hence corresponding keypoints would be prone to change in illumination. Image gradients are a better choice for constructing a feature descriptor, because they only affected by change of pixel intensity, rather than intensity values itself.
	
	\begin{figure}[h]
		\begin{center} 
			\includegraphics[width=15cm]{images/descriptor.jpg} 
			\caption{The left side shows a 8x8 neighbourhood of image gradients around a keypoint. The blue circle is supposed to indicate that each gradient is weighted by a 2D Gaussian function. As shown on the right These gradients are organized in 4 histograms each accumulating a 4x4 subregions. However the original version of SIFT by David Lowe uses a 16x16 neighbourhood, resulting in 16 histograms of 4x4 subregions. Image was taken from \cite{SIFT}} 
		\end{center} 
	\end{figure} 
	
	%relative gradient directions / rotated 16x16 neighbourhood 
	We use the scale of each keypoint to select the image from our Gaussian pyramid with the closest scale and compute all gradient directions and magitudes in a 16x16 pixel neighbourhood around that keypoint. All gradient directions are rotated relative to keypoint's orientation so that our feature descriptors achieve rotational invariance.
	
	The gradient neighbourhood is split up into 4x4 cells, so there will be a total of 16 cells. The Analogous to section 2.4 each cell accumulates its gradients in an orientation histogram, but this time with only 8 bins, which keeps the descriptor small. Each gradient is not only weighted by its magnitude, but also by a 2D Gaussian function with its origin at the keypoint's center. This means that gradients which are close to the center will have a stronger influence on the feature descriptor than those further away which might describe the background. This also helps with non-rigid objects, because central gradient directions around each keypoint are less likely to change. 
	
	The feature descriptor is actually just a sequence of linking all values of every histogram together in a consistent manner. Since every keypoint uses 4x4 cells with 8 orientation bins each, accumulating all these values in a single descriptor, results in a 128 dimensional vector. This vector is normalized to unit length in order to make it more comparable with other descriptors. 
	
	%Experiments show that normalizing the vector to unit length helps to cope with illumination changes. Have multiple strong orientations in our final descriptor is more desirable than having a single dominant one, because "non-linear illumination changes can also occur due to camera saturation or due to illumination changes that affect 3D surfaces with differing orientations by different amounts" \cite[p. 16]{SIFT}. 
	
	\section{Problem cases} 
	We have seen that SIFT is a sophisticated algorithm, but computing SIFT features on large images will take a fair amount of time. Herbert Bay addresses this problem in his publication "Speeded Up Robust Features" (SURF) from 2006 \cite{SURF}. His SURF features are based on SIFT, but can be computed much faster. Their smaller 64 dimensional descriptor also allows for faster matching of its features with only minor loss in accuracy. Hence they might be better suited for video analysis or some applications that require real-time computation.
	...
	Another difficult situtation might be if an picture shows multiple occurrences of the same object. For instance a fence consists of many pickets, which all have a similar shape and hence generate similar SIFT features, which only vary due to the diversity of the background. 
	Having repetitive structures in one scene might lead to false matches of correspondences, called outliers. %...
	
	Repetative structures are not the only 
